...
async function botLog(embed, severity) {
  ...
  embed.setColor(logColor).setTimestamp().setFooter({ text: "Logs" });
  try {
    ...
    console.warn(
      "ERROR: No Log Channel Environment Variable Found, Logging will not work."
    );
  }
}
...
bot.once("ready", async () => {
  ...
  botLog(
    new EmbedBuilder()
      .setDescription(`💡 R.Clops is now online! logged in as ${bot.user.tag}`)
...
bot.on("interactionCreate", async (interaction) => {
  if (interaction.isCommand()) {
    ...
    if (interaction.options !== undefined) {
      try {
        ...
        console.log(
          `WARNING: Unable to create arguments for legacy command '${interaction.commandName}', this may not affect modern slash commands: ${err}`
        );
      }
    }
    try {
      botLog(
        new EmbedBuilder().setDescription(
          `Command used by ${interaction.user.tag} - Command ` +
            "`" +
  ...
  if (interaction.isButton()) {
    botLog(
      new EmbedBuilder().setDescription(
        `Button triggered by user **${interaction.user.tag}** - Button ID: ${interaction.customId}`
      ),
      0
    );
...
bot.on("messageDelete", async (message) => {
  try {
    botLog(
      new EmbedBuilder()
        .setDescription(
          `Message deleted by user: ${message.author}` +
            "```" +
            `${message.content}` +
            "```"
        )
    ...
    botLog(
      new EmbedBuilder()
        .setDescription(
          `Something went wrong while logging a Deletion event: ${err}`
        )
...
bot.on("messageUpdate", (oldMessage, newMessage) => {
  if (
    ...
    oldMessage.author.id != process.env.CLIENTID
  ) {
    botLog(
      new EmbedBuilder()
        .setDescription(
          `Message updated by user: ${oldMessage.author}` +
            "```" +
            `${oldMessage}` +
            "```" +
            `Updated Message:` +
...
bot.on("guildMemberRemove", (member) => {
  ...
  botLog(
    new EmbedBuilder()
      .setDescription(
        `User ${member.user.tag}(${member.displayName}) has left or was kicked from the server.`
      )
      ...
      .addFields(
        ...
        {
          name: `Date Joined`,
          value: `<t:${(member.joinedTimestamp / 1000) >> 0}:F>`,
        },
...
process.on("uncaughtException", function (err) {
  ...
  bot.channels.cache
    .get(process.env.LOGCHANNEL)
    .send({ content: `⛔ Fatal error experienced: ${err}` });
});
